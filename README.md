...
1. 클라이언트에서 **카카오 로그인** 요청 → 카카오 OAuth2 인증 서버 연동
2. 인증 성공 시 서버가 카카오 계정(`kakao_id`)을 확인하고, 신규/기존 `user` 처리
3. **Access / Refresh JWT** 발급
4. 로그인 성공 후 → **딥링크(Custom Scheme)** 로 앱에 연결
   - 예시: `myapp://oauth/kakao?access=...&refresh=...`
   - 앱은 토큰을 추출해 DataStore에 저장하고 이후 모든 API 호출에 활용
...
- **서버**:
  ...
  - 외식/공복/술자리 패턴 리포트
- **DB(READ)**: `user`, `user_information`, `weight`, `diet(+diet_food)`, `routine(+exercise, set)`
...
- **생성**: `POST /ai/diets/create`
  ...
  - DB(WRITE): `diet(diet_entry_type=AI_PLAN)` + `diet_food`
...
- **API 명세서**: (http://15.165.158.249:8080/swagger-ui/index.html)
- **아키텍처 다이어그램**:
