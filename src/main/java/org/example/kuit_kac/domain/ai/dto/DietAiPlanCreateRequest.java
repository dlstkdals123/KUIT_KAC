...
public record DietAiPlanCreateRequest(
    ...
    List<@Valid DietAiFoodCreateRequest> foods
) {
    public static DietAiPlanCreateRequest from(User user, String dietType, LocalDate date, List<DietAiFoodCreateRequest> foods) {
        return new DietAiPlanCreateRequest(
            user.getId(),
            dietType,
            date,
            foods
